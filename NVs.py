...
def NV(seq, alphabet=list('ACGT'), with_gaps=True,
      d_weights=True):
    '''
    Natural Vectors
    Deng et al. 2011
...
def WMRV(seq, alphabet=list('ACGT')): 
    ...
    def SW(k): return 'W' if k in ['A', 'T'] else 'S'
    def MK(k): return 'M' if k in ['A', 'C'] else 'K'
    def RY(k): return 'R' if k in ['A', 'G'] else 'Y'
...
def kmerCount(seq, k, alphabet=list('ACGT')):
    words = [''.join(w) for w in product(alphabet, repeat=k)]
    seq = [''.join(w) for w in windowed(list(seq), k)]
    return {key:sum([1 for kmer in seq if kmer==key]) for key in words}
    
def kmerProb(seq, k):
    N = len(seq)
    return {key:float(val)/(N-k+1) for key, val in kmerCount(seq, k).items()}
...
def ENV(seq, k=5, intensity_range=255):
    ...
    N = len(seq)
    chaos = FCGR(seq, k)
    chaos = np.round(chaos/np.amax(chaos)*
                     intensity_range, 0).astype(int)
